...
class ConnectionManager:
    ...
    async def connect(self, uid: int, ws: WebSocket):
        await ws.accept()
        self.active[uid] = ws
        # Уведомляем чаты, что пользователь онлайн
        await self.broadcast_status(uid, "online")

    def disconnect(self, uid: int):
        if uid in self.active:
            del self.active[uid]
            # Уведомляем чаты, что пользователь офлайн (запускаем асинхронно)
    ...
    async def broadcast_status(self, user_id: int, status: str):
        """Оповещает все чаты, в которых состоит пользователь, о его статусе."""
        conn = sqlite3.connect("messenger.db", timeout=10)
        c = conn.cursor()
...
async def apply(user: PendingUser):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO pending_users (first_name,last_name,password,phone,created_at) VALUES (?,?,?,?,?)",
...
def login(data: LoginRequest):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id,first_name,last_name,avatar_url,theme,wallpaper_url,custom_theme,phone,bio FROM users WHERE id=? AND password=?",
...
def create_chat(data: CreateChat):
    ...
    conn = sqlite3.connect("messenger.db", timeout=10)
    c = conn.cursor()
    # Для личного чата имя будет содержать обоих участников (чтобы корректно отображать на клиенте)
    if len(data.user_ids) == 2:
        # Найдём имена участников
        c.execute("SELECT first_name, last_name FROM users WHERE id=? OR id=?", (data.user_ids[0], data.user_ids[1]))
        rows = c.fetchall()
        if len(rows) == 2:
            name = f"{rows[0][0]} {rows[0][1]}, {rows[1][0]} {rows[1][1]}"
        else:
            name = data.name or "Чат"
    else:
...
def get_messages(chat_id: str, limit: int = 50):
    conn = sqlite3.connect("messenger.db", timeout=10)
    c = conn.cursor()
    c.execute("SELECT sender_id,sender_name,text,file_url,file_type,timestamp FROM messages WHERE chat_id=? ORDER BY id DESC LIMIT ?", (chat_id, limit))
    msgs = [{"sender_id": r[0], "sender_name": r[1], "text": r[2], "file_url": r[3], "file_type": r[4], "timestamp": r[5]} for r in c.fetchall()]
    conn.close()
...
async def telegram_webhook(request: Request):
    ...
    if "callback_query" in data:
        ...
        if d.startswith("approve_"):
            ...
            try:
                approve_user(uid)
                await httpx.AsyncClient().post(url, json={"chat_id": cid, "message_id": mid, "text": f"✅ Заявка #{uid} одобрена!"})
            except:
                await httpx.AsyncClient().post(url, json={"chat_id": cid, "message_id": mid, "text": "❌ Ошибка"})
    ...
    try:
        while True:
            ...
            if msg_type == "message":
                ...
                # Рассылаем событие "печатает" всем в чате, кроме отправителя
